...
long EditDistance_Iterativ(char* A, size_t lengthA, char* B, size_t lengthB)
{
    ...
    size_t M = ctx.M ;
    size_t N = ctx.N ;
    /* Allocation and initialization of tab_phi_i to NOT_YET_COMPUTED*/
    long* tab_phi_i = (long *) malloc ( (N+1) * sizeof(long));
    if (tab_phi_i == NULL) { perror("EditDistance_Iterativ: malloc of tab_phi_i" ); exit(EXIT_FAILURE); }
    for (int j=0; j<=N; ++j) tab_phi_i[j] = NOT_YET_COMPUTED ;
    ...
    for (long j = N-1; j >= 0; j--) tab_phi_i[j] = (isBase(ctx.Y[j]) ? INSERTION_COST : 0) + tab_phi_i[j+1] ;

    /*On traite ligne par ligne*/
    for (long i = M - 1; i >= 0; i--){
        /* i < M, j = N*/
        long temp1 = (isBase(ctx.X[i]) ? INSERTION_COST : 0) + tab_phi_i[N] ;
        ...
        for (long j = N-1; j >= 0; j--){
            ...
            if (! isBase(ctx.X[i])){ /* skip ccharacter in Xi that is not a base */
                ...
                long cas2 = INSERTION_COST + tab_phi_i[j] ;      
            ...
            /*On n'a plus besoin de tab_phi_i[j+1] de i+1, on le mets à jour*/
            tab_phi_i[j+1] = temp1;
            temp1 = temp2;
        }
    ...
    long res = tab_phi_i[0]; 
    /* Deallocation of tab_phi_i */
    free( tab_phi_i ) ;
