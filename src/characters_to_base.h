 ...
 */
#define isBase(c)	( _base_match[(int) c] != SKIP_BASE )
 ...
 */
#define isUnknownBase(c)	( _base_match[(int) c] == UNKOWN_BASE )

/**
 * \def isSameBase(a,b)
 * \brief retuns 0 iff chars a and b are mapped to two different bases 
 */
#define isSameBase(a,b)	( _base_match[(int) a] == _base_match[(int) b] )
